...
// Note: booking_participants, payments and events for this booking
// are set to ON DELETE CASCADE in the schema.
...
// Creates the payment row and payment_logged event
// together inside one PostgreSQL transaction.
async function createPaymentForBooking(req, res) {
  ...
  try {
    ...
    const bookingCheck = await client.query(
      'SELECT id, trip_id FROM bookings WHERE id = $1',
      [req.params.bookingId]
    );
    ...
    // Check that the payer exists and belongs
    // to the same trip as the booking.
    const payerCheck = await client.query(
      `SELECT participants.id
       FROM participants
       JOIN bookings
         ON bookings.trip_id = participants.trip_id
    ...
    if (payerCheck.rows.length === 0) {
      ...
      return res.status(400).json({
        error: 'payer_id does not belong to the same trip as this booking'
      });
    }
    ...
    // Create the corresponding event using
    ...
    // Both payment and event succeeded
    ...
    // If payment or event creation fails,
